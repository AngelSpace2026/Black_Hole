...
def find_best_chunk_strategy(input_filename):
    """Finds the best chunk strategy for compression using fixed 255 values."""
    try:
    ...
    max_consecutive_no_improvements = 255  # Fixed value
    max_time_seconds = 255  # Fixed value

    while consecutive_no_improvements < max_consecutive_no_improvements:
        iteration += 1
        chunk_size = 255  # Fixed chunk size
        max_positions = file_size // chunk_size
        num_positions = min(255, max_positions)  # Always 255 positions if possible
    ...
    try:
        with open(compressed_filename, 'wb') as outfile:
            compressed_data = compress_with_paq(reverse_chunks_at_positions(file_data, best_chunk_size, best_positions), 
                                                best_chunk_size, best_positions, file_size)
...
def add_numbers_to_data(data):
    ...
    print(f"Performing operation: {operation} {random_number}")

    if operation == '+':
        data = bytearray((x + random_number) % 256 for x in data)
    else:
        data = bytearray((x - random_number) % 256 for x in data)
