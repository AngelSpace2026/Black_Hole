...
PAQ8PX_BINARY = "./paq8px"  # Path to the PAQ8PX binary

def ensure_paq8px_permissions():
    """Ensures that the PAQ8PX binary is executable."""
    if not os.path.exists(PAQ8PX_BINARY):
        raise FileNotFoundError(f"Error: PAQ8PX binary '{PAQ8PX_BINARY}' not found.")
    
    # Make sure the binary is executable
    if not os.access(PAQ8PX_BINARY, os.X_OK):
        print(f"Setting execute permissions for {PAQ8PX_BINARY}...")
        os.chmod(PAQ8PX_BINARY, 0o755)  # Set the file as executable
        print(f"Permissions set for {PAQ8PX_BINARY}.")
...
def main():
    ...
    ensure_paq8px_permissions()  # Ensure the PAQ8PX binary has the correct permissions
    ...
    if mode == 1:
        ...
        output_filename = input("Enter output file name (e.g., output.compressed.paq): ")
        num_attempts = 1
