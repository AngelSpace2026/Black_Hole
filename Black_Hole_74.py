...
def apply_random_transformation(data):
    """Applies random addition or subtraction with a value between 1 and 2^X-1."""
    X = random.randint(1, 255)  # Random exponent for transformation
    value = random.randint(1, (2**X) - 1)
    operation = random.choice([-1, 1])  # Randomly choose subtraction (-) or addition (+)
    transformed_data = bytes((b + (value * operation)) % 256 for b in data)
    return transformed_data, value, operation

def compress_with_paq(data, chunk_size, positions, original_size, transform_value, transform_operation):
    ...
    metadata = struct.pack(">I", original_size) + struct.pack(">I", chunk_size) + \
               ...
               struct.pack(">I", transform_value) + struct.pack(">b", transform_operation)
...
def decompress_and_restore_paq(compressed_filename):
    ...
    try:
        ...
        transform_value = struct.unpack(">I", decompressed_data[12 + num_positions * 4:16 + num_positions * 4])[0]
        transform_operation = struct.unpack(">b", decompressed_data[16 + num_positions * 4:17 + num_positions * 4])[0]

        transformed_data = decompressed_data[17 + num_positions * 4:]

        # Reverse transformation
        restored_data = bytes((b - (transform_value * transform_operation)) % 256 for b in transformed_data)

        # Reverse chunk modifications
...
def find_best_chunk_strategy(input_filename, max_consecutive_no_improvements=1000000, max_time_seconds=1000000):
    ...
    best_chunk_size = 1
    best_positions = []
    best_transform_value = 0
    best_transform_operation = 0
    ...
    while consecutive_no_improvements < max_consecutive_no_improvements and time.time() - start_time < max_time_seconds:
        ...
        transformed_data, transform_value, transform_operation = apply_random_transformation(reversed_data)

        compressed_data = compress_with_paq(transformed_data, chunk_size, positions, file_size, transform_value, transform_operation)
        ...
        if compression_ratio < best_compression_ratio:
            ...
            best_transform_value = transform_value
            best_transform_operation = transform_operation
            consecutive_no_improvements = 0
            print(f"Improved compression: {len(compressed_data)} bytes (chunk size: {chunk_size}, positions: {positions}, transform: {transform_operation} {transform_value})")
        else:
    ...
    print(f"Transformation: {best_transform_operation} {best_transform_value}")
    print(f"Time taken: {elapsed_time:.2f} seconds")

    compressed_filename = f"{input_filename}.compressed.bin"
    try:
        with open(compressed_filename, 'wb') as outfile:
            compressed_data = compress_with_paq(
                apply_random_transformation(reverse_chunks_at_positions(file_data, best_chunk_size, best_positions))[0],
                best_chunk_size, best_positions, file_size, best_transform_value, best_transform_operation)
