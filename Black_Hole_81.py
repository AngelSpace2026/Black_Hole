...
def move_bits_randomly(data, shift_min=1, shift_max=(2**32 - 1)):  
...
def find_best_iteration(input_filename, max_iterations):
    ...
    for iteration in range(max_iterations):
        ...
        # Determine the strategy based on divisibility checks
        if file_size % 4 == 0:  # Divisible by 4
            strategy = 4
        elif file_size % 3 == 0:  # Divisible by 3
            strategy = 3
        elif file_size % 2 == 0:  # Divisible by 2
            strategy = 2
        else:  # Not divisible by 2 (use strategy 1)
            strategy = 1

        # Apply the chosen strategy
        if strategy == 1:  # Strategy 1: Not divisible by 2
        ...
        elif strategy == 2:  # Strategy 2: Divisible by 2
            reversed_data = reverse_chunks_at_positions(file_data, chunk_size, positions)
            ...
            compression_ratio = len(compressed_data) / file_size
        elif strategy == 3:  # Strategy 3: Divisible by 3
            reversed_data = reverse_chunks_at_positions(file_data, chunk_size, positions)
            modified_data = add_random_bytes(reversed_data)
            compressed_data = compress_with_paq(modified_data, chunk_size, positions, file_size, 3)
            compression_ratio = len(compressed_data) / file_size
        elif strategy == 4:  # Strategy 4: Divisible by 4
            reversed_data = reverse_chunks_at_positions(file_data, chunk_size, positions)
            modified_data = move_bits_randomly(reversed_data)
            compressed_data = compress_with_paq(modified_data, chunk_size, positions, file_size, 4)
            compression_ratio = len(compressed_data) / file_size
