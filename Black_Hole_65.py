...
# Decompress and restore data (with 8-byte size handling)
def decompress_and_restore_paq(compressed_filename):
    ...
    # Unpack size format as 8 bytes (for all file sizes)
    original_size = struct.unpack(">Q", decompressed_data[:8])[0]  # Always 8-byte size
    decompressed_data = decompressed_data[8:]

    chunk_size = struct.unpack(">I", decompressed_data[:4])[0]  
    num_positions = struct.unpack(">I", decompressed_data[4:8])[0]  
    positions = list(struct.unpack(f">{num_positions}I", decompressed_data[8:8 + num_positions * 4]))  

    chunked_data = decompressed_data[8 + num_positions * 4:]  
