...
# Great Theorem of Fermat transformation function
def Root_theorema_ferma_great(value, iterations=255, exponent=3):
    """
    Applies transformations based on Fermat’s Last Theorem by raising values to a power
    and modifying them iteratively.
    """
    transformed_value = value

    for _ in range(iterations):
        # Apply power transformation based on exponent (default 3)
        transformed_value = transformed_value ** exponent

        # Check if Fermat's Last Theorem conditions hold
        a = random.randint(1, 100)
        b = random.randint(1, 100)
        c = int(transformed_value ** (1/exponent))  # Approximate root

        # If the equation a^n + b^n ≠ c^n holds, adjust the value
        if (a**exponent + b**exponent) != c**exponent:
            transformed_value = (a + b) * transformed_value - c

        # Ensure the value remains within a valid range
        transformed_value = abs(transformed_value) % (2**32)
...
def reverse_chunks(input_filename, reversed_filename, chunk_size, num_reversals, num_sets):
    ...
    for _ in range(num_reversals):
        if total_chunks > 1:
            positions = [Root_theorema_ferma_great(random.randint(0, total_chunks - 1)) % total_chunks for _ in range(min(num_sets, total_chunks))]
...
def main():
    ...
    if mode == 1:
        ...
        compressed_filename_base = input("Enter the base name of the compressed file to extract: ")
